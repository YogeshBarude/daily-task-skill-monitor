...
export const sprintColumns = ["Project", "Task", "Product Owner", "Work Hours", "Completion", "Task Given Date", "Due Date", "Status"];
...
export function sprintShareText(tasks: WorkTask[], weekStart: string) {
  ...
  const lines = tasks.map((task, index) =>
    `${index + 1}. ${task.projectName} | ${task.title} | PO: ${task.productOwner} | ${Math.round((task.actualMinutes / 60) * 10) / 10}h | ${task.completionPercentage}% | Given: ${task.receivedDate} | Due: ${task.dueDate} | ${task.status}`
  );
